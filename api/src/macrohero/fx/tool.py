"""LangChain @tool wrapper for the FX scenario projection engine.

The LLM does NOT supply factor magnitudes. It supplies a `direction` and a
`severity` tier per factor; the engine maps the tier to a shock magnitude
using the factor's own realized volatility scaled to the chosen horizon:

    shock = sign(direction) * SEVERITY_MULTIPLIER[severity] * sigma_daily * sqrt(h)

This anchors the numbers to actual market data instead of LLM guesswork.
"""
...
from macrohero.fx.factors import get_factor_by_name
from macrohero.fx.projection import ProjectionResult, fit_and_project

# Severity-tier multipliers applied to sigma_horizon. The tiers are sized so
# that "moderate" is a typical 1-sigma move at the horizon and "extreme" is a
# tail (~3-sigma) move. Keep these in sync with the system prompt's table.
...
class FactorShock(BaseModel):
    ...
    severity: Severity = Field(
        description=(
            "Magnitude tier at the chosen horizon. The engine sizes the move"
            " as a multiple of the factor's recent realized volatility:"
            " mild=0.5*sigma, moderate=1*sigma, severe=2*sigma, extreme=3*sigma."
            " Pick the tier that best matches your qualitative read of the scenario."
        )
    )
...
def _result_as_dict(result: ProjectionResult) -> dict[str, Any]:
    """Convert the dataclass to a plain dict for JSON serialization in the stream."""
    return {
        "pair": result.pair,
...
async def run_factor_projection_impl(args: RunFactorProjectionArgs) -> dict[str, Any]:
    """The deterministic projection. Returns a dict, never raises."""
    ...
    try:
        ...
        end = spot_date  # last close we have
        # 1.5x for typical weekend density, plus a 30-day pad to absorb holiday clusters
        # that bite small windows (e.g. regression_window_days=60).
    ...
    # Use a sentinel column name to avoid collision with a factor named "pair"
    # (no current factor has dunder edges, so __pair__ is safe).
    pair_col = "__pair__"
    ...
    # Keep last `regression_window_days` observations.
    ...
    # Compute transformed regressors per spec.
    ...
    for fs in args.factors:
        ...
        if spec.transform == "log_return":
            ...
            # yields published in %; bp = % * 100. Daily diff in bp:
    ...
    # Convert each LLM-supplied (direction, severity) pair into a numeric
    # expected_change anchored to the factor's own realized vol. This is the
    # core "LLM-doesn't-pick-numbers" guarantee.
    ...
    for i, fs in enumerate(args.factors):
        ...
        if factor_transforms[i] == "log_return":
            # sigma_horizon is in decimal; tool's user-facing unit is %, so x100.
            expected[fs.name] = sign * multiplier * sigma_horizon * 100.0
        else:  # abs_change_bp
            # sigma_horizon already in bp on both sides.
    ...
    # Backfill ticker + severity/direction/sigma_multiple on each contribution
    # so the FE can render the "severe down, -2 sigma" hint and prove the
    # magnitude came from market data, not from the LLM.
...
@tool(args_schema=RunFactorProjectionArgs)
async def run_factor_projection(
    pair: str,
    horizon_days: int,
    factors: list[FactorShock],
    regression_window_days: int = 252,
) -> dict[str, Any]:
    """Run a deterministic factor-sensitivity projection for an FX pair.

    Given the user's scenario (pair + horizon + factor shocks), fit an OLS
    regression of the pair on the factors over a recent window and project
    the pair's % move and 95% confidence band. Returns the structured result.
    """
